...
Create GitHub's AWS OIDC identity provider and restrict the deploy role trust policy to this repository's `main` branch subject and the `sts.amazonaws.com` audience. The role needs only ECR authorization/upload access for the three repositories plus permission to send `AWS-RunShellScript` to the deployment instance and read its command invocation. Do not configure long-lived AWS access keys in GitHub.
